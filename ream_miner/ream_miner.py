...
from refpa.utils_refpa import compute_clusters_for_all_input_graphs, evaluate_clusters, \
    compute_rm_graph_nodes, create_final_rm_graph

class ReamMiner:

    def __init__(self, path: str, threshod: float) -> None:
        """Constructor
    ...
    @property
    def threshold(self):
        """Method to get the threshold which is used to identify the common nodes
    ...
    def create_initial_rm_graphs(self) -> None:
        ...
        for i in range(0, number_of_views):
            ...
            mcc_views_algorithm.threshold = self.threshold
            mcc_views_algorithm.initiate_sets()
            mcc_views_algorithm.execute()
            mcc_views_algorithm.rm_graph.delete_node(root_node_id)
            common_nodes = list(mcc_views_algorithm.rm_graph.nodes.index)


            # reset threshold
            mcc_views_algorithm.threshold = mcc_threshold
    ...
    def execute(self) -> None:
        ...
        for i in range(0, len(self.__initial_graphs_views)):
            ...
            current_rm_graph_nodes = compute_rm_graph_nodes(current_input_views_common_nodes,
                                                            current_view_rm_clusters)
